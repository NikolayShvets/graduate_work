...
class Confirmation(BaseModel):
    ...
    return_url: HttpUrl | None = Field(
        default=None,
        description="URL-адрес, по которому пользователь вернется после подтверждения/отмены платежа на веб-странице."
    )
    ...
    type: str | None = Field(
        default=None,
        description="Значение: перенаправление."
    )
...
class OutputPaymentScheme(BaseModel):
    ...
    confirmation: Confirmation | None = Field(
        default=None,
        description="Выбранный сценарий подтверждения платежа."
    )
    description: str = Field(
        description="Описание транзакции."
    )
    metadata: Any = Field(
        description="Любые дополнительные данные, которые могут потребоваться вам для обработки платежей."
    )
    ...
    recipient: RecipientScheme = Field(
    ...
    paid: bool = Field(
        description="Атрибут оплаченного заказа."
    )
    test: bool = Field(
        description="Атрибут тестовой транзакции."
    )
    captured_at: datetime | None = Field(
        default=None,
        description="Время получения платежа."
    )
    expires_at: datetime | None = Field(
        default=None,
        description="Период, в течение которого можно бесплатно отменить или получить платеж."
    )
...
class OutputRefundScheme(BaseModel):
    ...
    status: StatusEnum = Field(
        description="Статус возврата платежа.",
        examples=[*StatusEnum]
    )
