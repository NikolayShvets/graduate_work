...
class Confirmation(Base):
    confirmation_url: HttpUrl | None = Field(
        default=None,
        description="URL-адрес, на который будет перенаправлен пользователь для "
...
class OutputPaymentScheme(Base):
    ...
    metadata: Any = Field(
        description="Любые дополнительные данные, которые могут потребоваться "
                    "вам для обработки платежей."
    )
    ...
    expires_at: datetime | None = Field(
        default=None,
        description="Период, в течение которого можно бесплатно отменить или "
                    "получить платеж.",
    )
