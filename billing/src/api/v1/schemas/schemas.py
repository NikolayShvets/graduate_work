...
class AutoPaymentScheme(NewPaymentScheme):
    payment_method_id: UUID = Field(description="ID первого платежа", examples=["255350c9-000f-5000-a000-1f211b3ea0a7"])
...
class OutputNewPaymentScheme(Base):
    ...
    payment_method_id: UUID | None = Field(description="Id главного платежа",
...
class InputRefundScheme(Base):
    payment_id: UUID = Field(description="ID платежа", examples=["255350c9-000f-5000-a000-1f211b3ea0a7"])
    amount: float = Field(description="Стоимость", examples=[100.00, 500.00])
    currency: CurrencyEnum = Field(description="Валюта", examples=[CurrencyEnum.RUB, CurrencyEnum.ENG])
