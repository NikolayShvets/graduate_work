...
async def fill_persons():
    ...
    async with postgresql.async_session() as session:
        for _ in range(7):
            obj = await person_repository.create(
                session, data={"full_name": faker.name()}
            )
            id_list.append(obj.id)
    return id_list
    ...
    films = [
        {
            ...
            "creation_date": faker.date_between(
                datetime.date(1950, 1, 1), datetime.date(2023, 1, 1)
            ),
        }
        for _ in range(7)
    ]
...
async def fill_genre_film_work(genre_id_list, film_id_list):
    ...
    async with postgresql.async_session() as session:
        for id_ in genre_fw_id_list:
            data = {"genre_id": id_[0], "film_work_id": id_[1]}
